...
$cellRange = 'A1:D1';//SELECCIONE DESDE LA COLUMA A-D DE LA FILA 1
...
//SELECCION DE TODOS LOS DATOS CONTENIDOS EN LA TABLA ESCRITA
...
ESTE CODIGO SE MODIFICA SEGUN COMO SE ENCUENTRE EN LA BASE DE DATOS (LN 73-76)
...
while ($fila = mysqli_fetch_assoc($resultado)) {
    ...
    $sheet->setCellValue('C' . $row, $fila['precio']);//A$row(2) (el correo que almacena la base de datos)
    $sheet->setCellValue('D' . $row, $fila['cantidad']);//A$row(2) (el usuario que almacena la base de datos)
    ...
    $sheet->getStyle('A' . $row . ':D' . $row)->getAlignment()->setVertical(Alignment::VERTICAL_CENTER);//CENTRAR LAS PALABRAS SENTIDO VERTICAL
    $sheet->getStyle('A' . $row . ':D' . $row)->getAlignment()->setWrapText(true);//AUTOAJUSTE A LA CELDA
    
    $row++;//A$row:D$row
}

$sheet->getStyle('A1:D' . $row)->getAlignment()->setHorizontal(\PhpOffice\PhpSpreadsheet\Style\Alignment::HORIZONTAL_CENTER);//CENTRAR LAS PALABRAS SENTIDO HORIZONTAL

$filename = "BD_videojuegos.xlsx";//Nombre del archivo
